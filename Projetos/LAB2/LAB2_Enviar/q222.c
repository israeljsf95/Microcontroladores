...
void INT2_Int() iv IVT_ADDR_INT2INTERRUPT{
     Delay_ms(100);
     //LATB = ~LATB;
     cont_int++;    // a cada vez que a interrupcao 2 e ativada cont_int incrementa 1
     if (cont_int > 3){
        cont_int = 1;
     }
     IFS1bits.INT2IF = 0; //interrupcao externa 2 e desativada
     TMR2 = 0;
     TMR3 = 0;
}
...
void janela_de_tempo() iv IVT_ADDR_T1INTERRUPT ics ICS_AUTO {
  ...
  //   IFS0bits.T2IF = 0;
    ...
    // IFS1bits.INT2IF = 0;
     ...
     //frequencia = TMR3*65536 + TMR2;
}

//Sistema de indicacao de problema incrementando uma serie de avisos atraves dos leds indicando que a frequencia medida esta aumentando
//e e necessario mudar a escala para que o equipamento de medição não seja danificado. Algo semelhante ao que acontece com as escalas de
//medicao de um multimetro, indicando visualmente como deve ser ligado para medir determinada faixa de tensao ou corrente!!!
...
void main() {
     ...
     ADPCFG = 0xFFFF;
     //Configuracao dos Registradores de Saida da interrupcao Externa
     TRISB = 0;
     TRISD = 0x0002;
     TRISE = 0x0000;
     TRISEbits.TRISE8 = 1;
