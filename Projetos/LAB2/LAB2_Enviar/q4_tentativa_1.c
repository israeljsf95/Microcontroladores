...
void GTMOint() iv IVT_ADDR_T1INTERRUPT ics ICS_AUTO {
     IFS0bits.T1IF = 0;  // zera a flag do timer após ela ser ativada
     distancia = TMR1;   // Distância em cm
...
void criativ() iv IVT_ADDR_INT0INTERRUPT ics ICS_AUTO {
     ...
     IFS0bits.INT0IF = 0; // interrupção é zerada e fica esperando ser acionada outra vez
}
...
int conversao(float distancia){
         ...
         centena1 = (dist_disp/100)%10;
           ...
           for(Ia=0; Ia<3; Ia++) {  // multiplexação para exibição dos valores nos displays
                     switch (Ia){
                            case(0): mux1 = 0;
                                     mux2 = 0;
                                     mux3 = 1;
...
void main() {
     ADPCFG = 0xFFFF;           // configura entradas e saídas como digitais;
     TRISB = 0x0000;            // PORTB como saída
     TRISCbits.TRISC13 = 0;     // RC13 como saída
     echo = 1;                  // T1CK como ENTRADA
     TRISD = 0x0000;            // PORTD como saída
     TRISEbits.TRISE0 = 0;      // RE0 como SAÍDA
     TRISEbits.TRISE1 = 0;      // RE1 como SAÍDA
     TRISEbits.TRISE2 = 0;      // RE2 como SAÍDA
     TRISEbits.TRISE3 = 0;      // RE3 como SAÍDA
     TRISEbits.TRISE8 = 1;      // RE8 como ENTRADA (é a interrupção)
     TRISF = 0x0000;            // PORTF como saída
     ...
     PR2 = 16000;               // usado pra que a função atraso_ms seja feita

     T1CON = 0x8070;            // ativamos o timer 1 e o prescaler fica em 256 p caber as contas
     T2CON = 0x8000;            // ativamos o timer 2 na função timer
     
     flag = 0;

     while(1){
              dist_cm = (distancia*0.272);  // distancia multiplicada pela conversão (256*62.5e-9*340/2)*100
              trigger = 1;                  // inicio do pulso de ativação do sensor
            ...
            if(flag == 0) {
              if ((dist_cm > 200)){
                 ...
                 rele1 = 1;  // relé1 desligado
                 rele2 = 1;  // relé2 desligado
                 rele3 = 1;  // relé3 desligado
                 rele4 = 1;  // relé4 desligado
              }
                      ...
                      rele4 = 0;
                       ...
                       rele1 = 0;  // relé1 sempre ligado
                       rele2 = 0;  // relé2 sempre ligado
                       rele3 = 0;  // relé3 sempre ligado
                       rele4 = 0;  // relé4 sempre ligado
                       criatividade = 0;
              }
            }
            else{
